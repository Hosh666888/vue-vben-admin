<template>
  <teleport :to="mergeProps.warpper">
    <div
      class="mars-dialog-thumb"
      v-show="isFold && show"
      ref="thumbnailRef"
      @click="toogleFold(false)"
    >
      <mars-icon :icon="mergeProps.thumbnail.icon" :width="20" color="#FFFFFF" />
    </div>
    <div
      class="mars-dialog"
      :class="[customClass, animationClass]"
      :style="{
        'padding-top': showHeader ? '44px' : '10px',
...
<script lang="ts" setup>
  import '/@/mars3d/components/mars-ui/base.less';
  import '/@/mars3d/components/mars-ui/function.less';
  import '/@/mars3d/components/mars-ui/index.less';
  ...
  const mergeProps = computed(() => {
    ...
    if (
      isAllowValue(newProps.closeable) &&
      (slots.title ||
        isAllowValue(newProps.title) ||
        isAllowValue(newProps.icon) ||
        newProps.draggable)
    ) {
  ...
  const showHeader = computed(
    () =>
      slots.title ||
      isAllowValue(mergeProps.value.icon) ||
      isAllowValue(mergeProps.value.title) ||
      mergeProps.value.draggable,
  );
    ...
    function handleMove(e: any) {
      if (handleName.indexOf('l') !== -1) {
        ...
        if (e.clientX > 0 && e.clientX < warpperEle.offsetWidth) {
          if (
            (dialogRef.value.offsetWidth <= mergeProps.value.minWidth && L > 0) ||
            dialogRef.value.offsetWidth > mergeProps.value.minWidth
          ) {
      ...
      if (handleName.indexOf('t') !== -1) {
        ...
        if (e.clientY > 0 && e.clientY < warpperEle.offsetHeight) {
          if (
            (dialogRef.value.offsetHeight <= mergeProps.value.minHeight && L > 0) ||
            dialogRef.value.offsetHeight > mergeProps.value.minHeight
          ) {
